...
export default class Application
{
    ...
    {
        this.gameUI = {}
        this.gameUI.speedDisplay = document.getElementById('speed-display')
        this.gameUI.totalDistance = 0
        this.gameUI.maxSpeed = 0
        this.gameUI.startTime = Date.now()
        
        this.time.on('tick', () =>
        {
            if(this.world && this.world.car && this.world.car.movement)
            {
                // Update speed display (convert to km/h and round)
                const speed = Math.abs(this.world.car.movement.localSpeed.x * 60) // Approximate conversion
                this.gameUI.speedDisplay.textContent = Math.round(speed)
                
                // Track max speed
                if(speed > this.gameUI.maxSpeed)
                {
                    this.gameUI.maxSpeed = speed
                }
                
                // Track distance traveled
                this.gameUI.totalDistance += Math.abs(this.world.car.movement.localSpeed.x * this.time.delta * 0.001)
            }
        })
        
        // Add keyboard shortcuts for stats
    ...
    setKeyboardShortcuts()
    {
        window.addEventListener('keydown', (event) =>
        {
            ...
            if(event.key === 'Tab')
            {
                ...
                const playTime = Math.round((Date.now() - this.gameUI.startTime) / 1000)
                const avgSpeed = this.gameUI.totalDistance > 0 ? (this.gameUI.totalDistance / playTime * 3.6) : 0
                
                alert(`Game Stats:
🚗 Total Distance: ${this.gameUI.totalDistance.toFixed(1)} units
⚡ Max Speed: ${Math.round(this.gameUI.maxSpeed)} km/h
📊 Average Speed: ${avgSpeed.toFixed(1)} km/h
⏱️ Play Time: ${Math.floor(playTime / 60)}:${(playTime % 60).toString().padStart(2, '0')}

Controls:
WASD/Arrows - Drive | Space/Ctrl - Brake | Shift - Boost
H - Horn | R - Reset Car | Tab - Stats`)
            }
        })
    }
